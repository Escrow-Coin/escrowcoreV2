...
#ifndef ESCO_CORE_FILEREPOSITORYMANAGER_H
#define ESCO_CORE_FILEREPOSITORYMANAGER_H
    ...
    void SubtractFile(unsigned int nBytesSize)
    {
        if (nFilesCount == 0 || nBytesSize > nBlockSize) {
            LogPrint("file", "%s - FILES. ERROR. Meta data blockfile info not valid: %d, bytes size total: %d, remove bytes: %d\n",
                     ...
                     nFilesCount,
                     nBlockSize,
                     nBytesSize); //TODO: ESCO 2 remove after debug
            return;
        }

        //TODO: ESCO 3 check match with file.vBytes.size() including additional sizes (e.g. bytes header)
...
#endif //ESCO_CORE_FILEREPOSITORYMANAGER_H
