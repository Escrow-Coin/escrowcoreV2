...
void PrivacyDialog::sendzPIV()
{
    ...
    // Clear zescrow selector in case it was used
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ESCO, ";
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("zESCO is currently disabled due to maintenance."));

        // Spend zESCO
        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("zESCO is currently disabled due to maintenance."));
    }
    else {
        // Mint zESCO
        ui->pushButtonMintzPIV->setEnabled(true);
        ui->pushButtonMintzPIV->setToolTip(tr("PrivacyDialog", "Enter an amount of ESCO to convert to zESCO", 0));
