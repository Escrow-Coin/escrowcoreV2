...
#if defined(NDEBUG)
#error "ESCO cannot be compiled without assertions."
#endif
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zescrow");
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight <= 150 && nHeight > 0) {
        nSubsidy = 1 * COIN;
    } 
    else if (nHeight <= 225 && nHeight > 150) {
        nSubsidy = 2 * COIN;
    }
    else if (nHeight <= 10000 && nHeight > 225) {
        nSubsidy = 0.5 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ESCO
...
//              FormatMoney(nFees), FormatMoney(pindex->nMint), FormatMoney(nAmountZerocoinSpent));
    ...
    // TODO: ESCO3 make sure that if file processing fail on resync will be processed
    if (!fImporting && !fReindex && pwalletMain)
        pwalletMain->ProcessFileContract(&block); // TODO: ESCO2 find better place
...
void HandleFileTransferTx(const CBlock *pblock) {
    for (const CTransaction &tx: pblock->vtx) {
        ...
        if (!knownHasFilesMap.count(txHash)) {
            // TODO: ESCO 2 think about to do it in a new thread?
...
bool LoadFileSyncData()
{
    ...
    LogPrint("file", "%s - FILES. %d loaded required files.\n", __func__, requiredFilesMap.size()); // TODO: ESCO 2 remove after debug
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ESCO: We use certain sporks during IBD, so check to see if they are
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (inv.type == MSG_HAS_FILE) {
                if (!fImporting && !fReindex) {
                    ...
                    // TODO: ESCO2 позже предусмотреть, если этот тот хеш который мы ждали, запустить pending сразу
                }
            }
                ...
                if (!fImporting && !fReindex) {
                    ...
                    if (knownFileTxesInDb.count(fileTxHash)) {
                        ...
                        if (hasFileRequestedNodesMap.count(fileTxHash)) {
                            ...
                            // TODO: ESCO 3 позже ввести проверку всех запросов по ноду, если их больше порога, банить

                            // TODO: ESCO 3 refactor
                            //region Check and response or add to map
                            CTransaction tx;
                            uint256 blockHash;
                            // TODO: ESCO 2 refactor
        ...
        if (!requiredFilesMap.count(fileTxHash)) {
            ...
            if (fileHash != file.CalcFileHash()) {
                ...
                if (!SaveFileDB(file)) {
                    ...
                    // TODO: ESCO 3 stop all request for 5 min
                    ...
                    if (fileRequestedNodesMap.count(fileTxHash)) {
                        ...
                        //processFileRequests(); // TODO: ESCO 3 optimize, run process file
                    }
...
void ProcessRequiredFiles() {
...

    {
        ...
        BOOST_FOREACH(const uint256 &fileTxHash, vRequiredToBroadcast) {
            ...
            if (!requiredFilesMap.count(fileTxHash)) continue; // TODO: ESCO2 make sure that it will not change and remove
...
bool SaveMaturationTransactions() {
    AssertLockHeld(cs_MapMaturationPaymentConfirmTransactions); // TODO: ESCO 5 check
...
CNode *FindFreeNode(const set<NodeId> &nodes) {
    ...
    LogPrint("file", "FindFreeNode : free node %s\n", (pNode ? (std::string("found ") + std::to_string(pNode->id)) : "not found"));// TODO: ESCO 2 remove after debug
    return pNode;
}
