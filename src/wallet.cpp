...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zescrow
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    if (GetBoolArg("-escrowstake", true)) {
    ...
    //zESCO
    if (GetBoolArg("-zescrowstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZescrowBalance = GetZerocoinBalance(false);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZPIVOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zESCO output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to ESCO
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    try {
        ...
        //Add the coin spend into a ESCO transaction
...
string CWallet::GetUniqueWalletBackupName(bool fzpivAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzpivAuto ? "-autozescrowbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZPivBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozescrowbackup-%d.dat", i);
        ...
        if (filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozescrowbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozescrowbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zescrowbackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zescrowbackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    receipt.SetStatus("Spend Successful", ZPIV_SPEND_OKAY);  // When we reach this point spending zESCO was successful
...
bool CWallet::ProcessFileContract(const CBlock* pblock) {
    ...
    for (auto it = mapMaturationPaymentConfirmTransactions.begin(); it != mapMaturationPaymentConfirmTransactions.end(); ) {
        ...
        if (blockHeight >= (it->second.blockHeight + FILE_PAYMENT_MATURITY)) {
            // TODO: ESCO 3 implement statuses, if state_error don't remove transaction from map
...
bool CWallet::OnPaymentConfirmed(const CTransaction& tx) {
    ...
    // TODO: ESCO 3 check if file already sent and triggered event processed correctly
    // TODO: ESCO 3 sync file sent status on blocks resync
    bool isSent = false;
    ...
    dbFile.fileExpiredDate = GetAdjustedTime() + paymentConfirm->nLifeTime; // TODO: ESCO 4 check lifetime and fee before
    encryptedFile.clear();
    ...
    // TODO: ESCO 3 remove after debug
...
bool CWallet::SendFileTx(const CFile& file, const CFileMeta& fileMeta, const CTxDestination& dest, uint256& outFileTxHash) {
    ...
    // send transaction
    {
        ...
        if (!wallet->CommitTransaction(newTx, reservekey, (useSwiftTX) ? "ix" : "tx"))
            return error("%s : Failed to CommitTransaction", __func__); //TransactionCommitFailed; // TODO: ESCO2 return detailed result
    }
