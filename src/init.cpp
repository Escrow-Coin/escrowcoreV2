...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12929, 12931));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ESCO specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-escrowstake=<n>", strprintf(_("Enable or disable staking functionality for ESCO inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zescrowstake=<n>", strprintf(_("Enable or disable staking functionality for zESCO inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzescrow=<n>", strprintf(_("Enable automatic wallet backups triggered after each zESCO minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zescrowbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zESCO backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 12928, 12930));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), 2014)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The ESCO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that ESCO is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ESCO Core is shutting down."));
...
#endif
    // Make sure only a single ESCO process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ESCO Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //ESCO: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //ESCO specific: zerocoin and spork DB's
                zerocoinDB = new CZerocoinDB(0, false, fReindex);
                pSporkDB = new CSporkDB(0, false, false);

                //ESCO specific: file DB's
                ...
                // ESCO: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // ESCO: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ESCO Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ESCO Core to complete") << "\n";
        ...
        bool fEnableZPivBackups = GetBoolArg("-backupzescrow", true);
...
//    if (nAnonymizeEscrowAmount < 2) nAnonymizeEscrowAmount = 2;
    ...
    LogPrintf("Anonymize ESCO Amount %d\n", nAnonymizeEscrowAmount);
